const AVAILABLE_WEEK_DAYS = ['Domingo', 'Segunda', 'Terca', 'Quarta', 'Quinta', 'Sexta', 'Sabado'];
...
class CALENDAR {
    constructor(options) {
        ...
        this.elements = {
            ...
            todayBtn: this.getFirstElementInsideIdByClassName('today-btn')
        };
    ...
    drawDays() {
        ...
        let latestDaysInPrevMonth = this.range(calendar.active.startWeek).map((_day, idx) => {
            return {
        ...
        let daysInActiveMonth = this.range(calendar.active.days).map((_day, idx) => {
            let dayNumber = idx + 1;
            let today = new Date();
            return {
                dayNumber,
        ...
        let daysTemplate = "";
        days.forEach(day => {
            daysTemplate += `<li id="${day.dayNumber}" class="${day.currentMonth ? '' : 'another-month '}${day.today ? 'active-day' : ''}${day.selected ? ' selected-day' : ''}${day.hasReserve ? ' reserve-day' : ''}" data-day="${day.dayNumber}" data-month="${day.month}" data-year="${day.year}" value="${day.dayNumber}"></li>`
        });
    ...
    drawMonths() {
        ...
        availableMonths.forEach((month, idx) => {
            monthTemplate += `<li id="${idx === calendar.active.month ? 'mesCalandario' : ''}" class="${idx === calendar.active.month ? 'active' : ''}" data-month="${idx}" value="${idx + 1}">${month}</li>`
        });
